...
class Poly2LogitESF(ModelBuilder):
    # Give the model a name
    _model_type = "Poly2LogitESF"
    ...
    def build_model(self, X: pd.DataFrame, y: pd.Series, **kwargs):
        ...
        with pm.Model(coords=self.model_coords) as self.model:
            ...
            mu_lbda_prior = self.model_config.get("mu_lbda")
            ...
            a2 = pm.Normal("a2", mu=a2_mu_prior, sigma=a2_sigma_prior)
            sigma_y = pm.InverseGamma(
                "sigma_y", alpha=sigma_y_alpha_prior, beta=sigma_y_beta_prior
            )
            # Logit prior
            D = self.model_config.get("n_eigenvectors")
            lbdai = pm.LogitNormal("lbdai", mu_lbda_prior, sigma_lbda_prior, shape=(D,))
            scalei = lbdai * tau
            bi = pm.Normal("bi", bi_mu_prior, scalei, shape=(1, D)).T

            # model
            mu = a0 + x_data * a1 + x_data**2 * a2 + pm.math.dot(Ei_data, bi)
            mu = mu[:, 0]
